...
import { ChevronRight, Star, Clock, Book, Play, Gamepad2 } from 'lucide-react';
      ...
      <div className="flex-1 bg-gray-50 dark:bg-chrono-card px-4 py-16 transition-colors">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <FeatureCard
            ...
            title="Techs & Magias"
            description="Lista de Double e Triple Techs, incluindo as secretas."
            link="/techs"
          />
          <FeatureCard
            icon={<ChevronRight className="text-chrono-red" size={32} />}
            title="Todos os Finais"
            description="Guia detalhado de como obter os 13 finais diferentes."
            link="/endings"
          />
          <FeatureCard
            icon={<Play className="text-pink-500" size={32} />}
            title="Jukebox Musical"
            description="Ouça a trilha sonora épica enquanto lê sobre sua história."
            link="/jukebox"
            highlight
          />
          <FeatureCard
            icon={<Gamepad2 className="text-purple-500" size={32} />}
            title="Quiz do Guardião"
            description="Teste seus conhecimentos e prove que é um fã de verdade."
            link="/quiz"
            highlight
          />
          <FeatureCard
            icon={<Book className="text-blue-500" size={32} />}
            title="Legado & Versões"
            description="Conheça as sequências, romhacks e diferenças entre versões."
            link="/versions"
          />
        </div>
      </div>
...
const FeatureCard: React.FC<{ icon: React.ReactNode, title: string, description: string, link: string, highlight?: boolean }> = ({ icon, title, description, link, highlight }) => (
  <Link to={link} className="block group h-full">
    <div className={`
      bg-white dark:bg-chrono-dark p-8 rounded-2xl border 
      ${highlight ? 'border-chrono-blue/50 ring-1 ring-chrono-blue/20' : 'border-gray-200 dark:border-gray-700'} 
      hover:border-chrono-blue hover:shadow-lg dark:hover:border-chrono-blue transition-all h-full hover:-translate-y-1 flex flex-col
    `}>
      <div className={`
        mb-4 w-16 h-16 rounded-xl flex items-center justify-center transition-colors
        ${highlight ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-gray-100 dark:bg-gray-800 group-hover:bg-blue-50 dark:group-hover:bg-gray-700'}
      `}>
        {icon}
      </div>
